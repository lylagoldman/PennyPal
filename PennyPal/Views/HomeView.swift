...
struct HomeView: View {
    ...
    @State private var selectedStatistic: StatisticMode = .income
        ...
        ScrollView(showsIndicators: true) {
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    ExpensesToggle(selectedStatistic: $selectedStatistic)
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Button {
                        ...
                        Text(entryType == .income ? "Add Income" : "Add Expense")
                            ...
                            .padding(.vertical, 18)
                            .background(Color.mediumMint, in: RoundedRectangle(cornerRadius: 16, style: .continuous))
                    }
                    .buttonStyle(.plain)
                }
                .dashboardCard()
                        ...
                        ForEach(data) { point in
                            VStack(spacing: 10) {
                                ...
                                HStack(alignment: .bottom, spacing: 6) {
                                    if selectedStatistic != .both {
                                        // Show only income when in .income mode
                                        RoundedRectangle(cornerRadius: 2, style: .continuous)
                                            .fill(Color.mediumMint)
                                            .frame(
                                                width: 16,
                                                height: max(6, CGFloat(point.income / maxValue) * (geometry.size.height - 28))
                                            )
                                    } else {
                                        // Show both income and expense side-by-side
                                        RoundedRectangle(cornerRadius: 2, style: .continuous)
                                            .fill(Color.mediumMint)
                                            .frame(
                                                width: 16,
...
private struct ExpensesToggle: View {
    @Binding var selectedStatistic: StatisticMode
    @State private var isExpensesOn = false

    var body: some View {
        HStack() {
            Toggle("", isOn: $isExpensesOn)
                .labelsHidden()
                .tint(Color(.mutedRed))
            .toggleStyle(.switch)
            .padding(.trailing, 5)
        ...
        .onAppear {
            // Initialize switch from current selection
            isExpensesOn = (selectedStatistic != .income)
        }
        .onChange(of: isExpensesOn) { _, newValue in
            selectedStatistic = newValue ? .both : .income
        }
    }
}
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            Text(summary.remaining.currencyText(symbol: currencySymbol))
                .font(.headline.weight(.bold))
                .foregroundStyle(summary.remaining >= 0 ? Color.mediumMint : Color.mutedRed)
        }
        .padding(16)
    ...
    private func entryButton(title: String, type: TransactionType) -> some View {
        Button {
            ...
            Text(title)
                ...
                .background(
                                entryType == type ?
                                (entryType == .expense ? Color.mutedRed.opacity(0.8) : Color.mediumMint) :
                                Color.white
                            )
                .clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous))
                .overlay(
                    RoundedRectangle(cornerRadius: 16, style: .continuous)
                        .stroke( entryType == type ? Color.clear : Color.charcoal.opacity(0.3), lineWidth: entryType == type ? 0 : 2)
                )
        }
        .buttonStyle(.plain)
    }
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            Text(transaction.amountText(symbol: currencySymbol))
                .font(.headline.weight(.bold))
                .foregroundStyle(transaction.isIncome ? Color.darkerMint : Color.mutedRed)
        }
        .padding(16)
